...
// 获取权限列表
router.get('/api/permissions', checkPermissions(['view_permissions']), (req, res) => {
  try {
...
// 新增权限
router.post('/api/permissions', checkPermissions(['create_permission']), (req, res) => {
  try {
    let permissions = []
    if (fs.existsSync(permissionsFile)) {
...
// 删除权限
router.delete('/api/permissions/:name', checkPermissions(['delete_permission']), (req, res) => {
  try {
    let permissions = []
    if (fs.existsSync(permissionsFile)) {
      permissions = JSON.parse(fs.readFileSync(permissionsFile, 'utf-8'))
