...
class ChatController {
  ...
  sendMessage = async (req, res) => {
    try {
      ...
      const { message, context, service, model } = req.body;
      ...
      const stream = aiServiceManager.sendMessage(userId, message, context, options);
      let chunkCount = 0;
      ...
      if (error.message.includes('fetch failed') || error.message.includes('Headers Timeout')) {
        ...
        errorCode = 503
      } else if (error.message.includes('Ollama 连接失败')) {
        errorMessage = '本地AI服务不可用，请检查Ollama服务状态'
        errorCode = 503
  ...
  switchService = async (req, res) => {
    try {
      ...
      const success = aiServiceManager.setService(service);
      
      if (success) {
        // 测试新服务的连接
        const testResult = await aiServiceManager.testService(service);
        ...
        res.json({
          ...
          message: `已切换到 ${aiServiceManager.getServiceDisplayName(service)}`
        });
      } else {
        res.status(400).json({
          code: 400,
